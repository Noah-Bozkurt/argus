...
impl HelperApi {
    ...
    pub fn from_env() -> Self {
        let values = std::env::var("ARGUS_ALLOWED_SERVICES").unwrap_or_else(|_| "nginx.service".into()).split(',').map(str::trim).filter(|s| !s.is_empty()).map(ToOwned::to_owned).collect::<Vec<_>>();
        Self::from_allowlist(values)
    }
    pub fn validate_service_name(service: &str) -> Result<(), HelperError> {
        let valid = service.ends_with(".service") && !service.is_empty() && service.len() <= 255 && service.chars().all(|c| c.is_ascii_alphanumeric() || matches!(c, '-' | '_' | '.' | '@'));
    ...
    async fn service_action(&self, action: &str, service: &str) -> Result<(), HelperError> {
        ...
        if !self.allowlisted_services.contains(service) { return Err(HelperError::ServiceNotAllowlisted); }
    ...
    pub async fn reboot(&self) -> Result<(), HelperError> {
        // systemd may tear down the process before a response can be returned. The control plane
        // therefore verifies reboot success after the next heartbeat using the uptime reset.
...
async fn run(program: &str, args: &[&str]) -> Result<(), HelperError> {
    ...
    if output.status.success() { Ok(()) } else { Err(HelperError::SystemCommandFailed(String::from_utf8_lossy(&output.stderr).trim().to_string())) }
}
...
mod tests {
    ...
    async fn blocks_non_allowlisted_service_before_execution() {
        ...
        assert!(matches!(helper.start_service("docker.service").await, Err(HelperError::ServiceNotAllowlisted)));
        assert!(matches!(helper.stop_service("docker.service").await, Err(HelperError::ServiceNotAllowlisted)));
    }
}
