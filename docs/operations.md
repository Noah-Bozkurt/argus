...
After `product`, `sudo -E ./scripts/first-server-acceptance.sh content` uses the installed Payload service's internal Argus API for that same personal Project. It requires the Project synchronization to exist, creates a public content model, proves a draft is absent from anonymous reads, publishes the record and verifies its sanitized public response. The script executes inside the Content container only to reach the private service port; it uses the supported HTTP API and does not mutate PostgreSQL directly.
