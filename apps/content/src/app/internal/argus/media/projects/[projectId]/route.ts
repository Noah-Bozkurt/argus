...
export async function POST(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    const created = await payload.create({
      collection: 'media', depth: 0, overrideAccess: true,
    ...
    console.error('Argus media upload failed', { projectId, userId: identity.userId, error })
...
export async function PATCH(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const updated = await payload.update({ collection: 'media', id: mediaId, depth: 0, overrideAccess: true, data: metadata })
