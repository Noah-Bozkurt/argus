...
export async function GET(request: Request, { params }: { params: Promise<{ projectId: string; formId: string }> }) {
  ...
  const first = await payload.find({ collection: 'form-submissions', depth: 0, limit: 500, page: 1, sort: 'submittedAt', overrideAccess: true,
  ...
  for (let page = 2; page <= first.totalPages; page += 1) {
    const next = await payload.find({ collection: 'form-submissions', depth: 0, limit: 500, page, sort: 'submittedAt', overrideAccess: true,
      where: { and: [{ project: { equals: project.id } }, { form: { equals: form.id } }] } })
    docs.push(...next.docs)
  }
  const fields: FormField[] = (form.fields ?? []).map((raw) => ({ key: String(raw.key ?? ''), label: String(raw.label ?? ''), type: String(raw.type ?? 'text') as FormField['type'], required: raw.required === true,
  ...
  return new NextResponse(csv, { headers: { 'content-type': 'text/csv; charset=utf-8', 'content-disposition': `attachment; filename="${String(form.slug ?? 'form')}-submissions.csv"`, 'cache-control': 'private, no-store', 'x-content-type-options': 'nosniff' } })
}
