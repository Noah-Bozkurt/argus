  ...
  try {
    if (body.operation === 'create_form') {
      ...
      const created = await payload.create({
        collection: 'form-definitions', depth: 0, overrideAccess: true,
        data: {
    ...
    if (body.operation === 'update_form_status') {
      ...
      const updated = await payload.update({ collection: 'form-definitions', id: formId, depth: 0, overrideAccess: true, data: { status: status as 'draft' | 'published' | 'archived' } })
    ...
    if (body.operation === 'update_submission_status') {
      ...
      const updated = await payload.update({ collection: 'form-submissions', id: submissionId, depth: 0, overrideAccess: true, data: { status: status as 'new' | 'reviewed' | 'spam' | 'archived' } })
    ...
    console.error('Argus form operation failed', { operation: body.operation, projectId, userId: identity.userId, error })
