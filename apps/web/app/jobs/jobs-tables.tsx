...
export default function JobsTables({ jobs, schedules }: { jobs: BackgroundJobView[]; schedules: JobScheduleView[] }) {
  ...
  return (
    <>
      <section className="resource-section" style={{ marginBottom: 14 }}>
        <div className="section-bar resource-toolbar-header">
          <div><h2>Schedules</h2><p>{visibleSchedules.length} of {schedules.length} recurring operations</p></div>
          <div className="resource-toolbar"><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search operations…" aria-label="Search operations and schedules" /></div>
        </div>
        {visibleSchedules.length === 0 ? <div className="empty-state"><strong>No matching schedules</strong>Feature-owned schedules will appear here when configured.</div> : (
          <div className="table-wrap" style={{ border: 0, borderRadius: 0 }}>
            <table className="responsive-table">
              <thead><tr><th>Operation</th><th>Scope</th><th>Resource</th><th>Interval</th><th>Status</th><th>Next run</th></tr></thead>
              <tbody>{visibleSchedules.map((schedule) => (
                <tr key={schedule.id}>
                  ...
                  <td data-label="Status"><span className={`state-label ${schedule.enabled ? 'success' : ''}`}>{schedule.enabled ? 'Enabled' : 'Disabled'}</span></td>
      ...
      <section className="resource-section">
        <div className="section-bar resource-toolbar-header">
          <div><h2>Recent operations</h2><p>{visibleJobs.length} of {jobs.length} executions · payloads stay hidden by design</p></div>
          <div className="resource-toolbar">
            <select value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)} aria-label="Filter operation status"><option value="all">All statuses</option><option value="QUEUED">Queued</option><option value="RUNNING">Running</option><option value="SUCCEEDED">Succeeded</option><option value="DEAD">Failed</option></select>
            <select value={sort} onChange={(event) => setSort(event.target.value as SortChoice)} aria-label="Sort operations"><option value="recent">Recently updated</option><option value="attempts">Attempts</option><option value="project">Project</option><option value="kind">Operation kind</option></select>
          </div>
        </div>
        {visibleJobs.length === 0 ? <div className="empty-state"><strong>No matching operations</strong>Change the search or status filter to show other executions.</div> : (
          <div className="table-wrap" style={{ border: 0, borderRadius: 0 }}>
            <table className="responsive-table">
              <thead><tr><th>Operation</th><th>Project</th><th>Resource</th><th>Status</th><th>Attempts</th><th>Run at</th><th>Action</th></tr></thead>
              <tbody>{visibleJobs.map((job) => (
                <tr key={job.id}>
                  ...
                  <td data-label="Status"><span className={`state-label ${statusClass(job.status)}`}>{job.status === 'DEAD' ? 'FAILED' : job.status}</span></td>
